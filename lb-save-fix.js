...
function currentValues(){
 ...
 return {items,subtotal,pct,discount,gstRate,gst,total,
  customer:(document.getElementById('customer')?.value||'Customer').trim(),
  phone:(document.getElementById('customerPhone')?.value||'').trim(),
  table:(document.getElementById('table')?.value||'-').trim(),
  orderType:(document.getElementById('orderType')?.value||'Dine In').trim(),payment:window.payment||'Cash'};
}
...
async function finalize(mode){
 ...
 // This is the ONLY place where a completed bill is added to sales.
 LB.sales=Array.isArray(LB.sales)?LB.sales:[];
 LB.sales.push(sale);
 try{LB.save('lb_sales_v2',LB.sales)}catch(e){}
 // Save to shared cloud before printing/WhatsApp. If cloud fails, keep the bill on screen.
 if(typeof window.lbCloudPush==='function'){
   const cloudOK=await window.lbCloudPush();
   if(!cloudOK){LB.sales=LB.sales.filter(x=>x.id!==sale.id);try{LB.save('lb_sales_v2',LB.sales)}catch(e){};LB.toast?.('Bill was not saved. Please try again.');return}
 }
 if(mode==='print'){
