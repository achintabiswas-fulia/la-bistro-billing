const CACHE='la-bistro-billing-v15';
...
function cleanAccidentalCode(html){
 const injection=`<script>(function(){function clean(root){const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);const a=[];let n;while(n=w.nextNode())a.push(n);a.forEach(n=>{const p=n.parentElement;if(!p||p.tagName==='SCRIPT'||p.tagName==='STYLE')return;const s=n.nodeValue||'';if(s.includes('function printReceipt()')||s.includes('function sendWhatsAppBill()')||s.includes('function bluetoothPrinter()'))n.remove();});}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>clean(document.body));else clean(document.body);})();<\/script>`;
